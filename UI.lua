...
-- Library functions 
    -- Misc functions
        local function is_primary_press(input)
            local t = input.UserInputType
            return t == Enum.UserInputType.MouseButton1 or t == Enum.UserInputType.Touch
        end

        local function is_primary_release(input)
            local t = input.UserInputType
            return t == Enum.UserInputType.MouseButton1 or t == Enum.UserInputType.Touch
        end

        local function is_pointer_move(input)
            local t = input.UserInputType
        ...
        function library:bind_activation(button, callback)
            local signal = button.Activated or button.MouseButton1Click or button.MouseButton1Down
            if not signal then
                return
            end
        ...
        function library:apply_responsive_scale(frame, base_size)
            local ui_scale = frame:FindFirstChildOfClass("UIScale")
            ...
            local function update()
                ...
                local min_dim = min(vp.X, vp.Y)
                local scale = 1

                -- Phone-only downscale. Tablet/PC keep scale=1.
                if min_dim < 700 then
                    local margin = 20
                    local sx = (vp.X - margin * 2) / (base_size.X.Offset > 0 and base_size.X.Offset or 1)
                    local sy = (vp.Y - margin * 2) / (base_size.Y.Offset > 0 and base_size.Y.Offset or 1)
                    scale = clamp(min(sx, sy, 1), 0.65, 1)
                end

                ui_scale.Scale = scale

                -- Keep centered after scale/orientation changes.
                frame.Position = dim_offset((vp.X - frame.AbsoluteSize.X) / 2, (vp.Y - frame.AbsoluteSize.Y) / 2)
            end
            ...
            if cam and cam.GetPropertyChangedSignal then
                library:connection(cam:GetPropertyChangedSignal("ViewportSize"), update)
            end
        ...
        function library:resizify(frame) 
            ...
            Frame.InputBegan:Connect(function(input)
                if is_primary_press(input) then
                    resizing = true
                    active_type = input.UserInputType
                    start = input_position(input)
                    start_size = frame.Size
                end
            end)

            Frame.InputEnded:Connect(function(input)
                if resizing and is_primary_release(input) and input.UserInputType == active_type then
                    resizing = false
                    active_type = nil
                end
            end)
            ...
            library:connection(uis.InputChanged, function(input, game_event) 
                if resizing and ((active_type == Enum.UserInputType.MouseButton1 and input.UserInputType == Enum.UserInputType.MouseMovement) or (active_type == Enum.UserInputType.Touch and input.UserInputType == Enum.UserInputType.Touch)) then
                    ...
                    local max_x = viewport_x / (scale > 0 and scale or 1)
                    local max_y = viewport_y / (scale > 0 and scale or 1)
        ...
        function library:draggify(frame)
            return library:draggify_handle(frame, frame)
        end

        function library:draggify_handle(frame, handle)
            ...
            handle.InputEnded:Connect(function(input)
                if dragging and is_primary_release(input) and input.UserInputType == active_type then
                    dragging = false
                    active_type = nil
                end
            end)
    ...
    -- Library element functions
        function library:window(properties)
            ...
            local items = cfg.items; do
                ...
                items[ "ui_scale" ] = library:apply_responsive_scale(items[ "main" ], cfg.size)
            ...
            do -- Other
                -- Drag only from the sidebar header so controls don't drag the whole UI.
                ...
                library:draggify_handle(items[ "main" ], items[ "drag_handle" ])

                -- Resizing is mouse-only (prevents touch getting "stuck").
        ...
        function library:tab(properties)
            ...
            local items = cfg.items; do 
                ...
                -- Multi Sections
                    ...
                    for _, section in cfg.tabs do
                        ...
                        library:bind_activation(multi_items[ "button" ], function()
                            data.open_page() 
                        end)
            ...
            library:bind_activation(items[ "button" ], function()
                cfg.open_tab()
            end)
        ...
        function library:toggle(options) 
            ...
            library:bind_activation(items[ "toggle" ], function()
                cfg.enabled = not cfg.enabled 
                cfg.set(cfg.enabled)
            end)

            library:bind_activation(items[ "toggle_button" ], function()
                cfg.enabled = not cfg.enabled 
                cfg.set(cfg.enabled)
            end)
        ...
        function library:slider(options) 
            ...
            library:connection(uis.InputChanged, function(input)
                if cfg.dragging and is_pointer_move(input) then
                    update_from_input(input)
                end
            end)

            library:connection(uis.InputEnded, function(input)
                if cfg.dragging and is_primary_release(input) then
                    cfg.dragging = false
        ...
        function library:dropdown(options) 
            ...
            library:bind_activation(items[ "dropdown" ], function()
        ...
        function library:colorpicker(options) 
            ...
            library:bind_activation(items[ "colorpicker" ], function()
                cfg.open = not cfg.open 
            ...
            library:connection(uis.InputChanged, function(input)
                if (dragging_sat or dragging_hue or dragging_alpha) and is_pointer_move(input) then
                    cfg.update_color(input)
                end
            end)

            library:connection(uis.InputEnded, function(input)
                if is_primary_release(input) then
                    dragging_sat = false
                    dragging_hue = false
        ...
        function library:keybind(options) 
            ...
            local items = cfg.items; do 
                ...
                -- Mode Holder
                    ...
                    for _, option in options do                        
                        ...
                        library:bind_activation(name, function()
                            cfg.set(option)
            ...
            library:bind_activation(items[ "keybind_holder" ], function()
                task.wait()
                items[ "key" ].Text = "..."	
        ...
        function library:settings(options)  
            ...
            library:bind_activation(items[ "tick" ], function()
                cfg.open = not cfg.open
        ...
        function library:init_config(window) 
            ...
            section:keybind({
                ...
                mode = "Toggle",
                key = (uis.TouchEnabled and "NONE") or Enum.KeyCode.RightShift,
                default = not uis.TouchEnabled,
            })
        end
    --
